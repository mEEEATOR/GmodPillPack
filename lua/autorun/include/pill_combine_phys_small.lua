...
pk_pills.register("rollermine",{
	...
	attack={
		...
		func= function(ply,ent)
			...
			ent:SetModel(ent:GetModel()==ent.formTable.model&&ent.formTable.model2||ent.formTable.model)
		end
	},
	attack2={
		mode="trigger",
	...
	moveSoundControl=function(ply,ent)
		...
		if MineSpeed>50 then
			return math.Clamp(MineSpeed/2,100,150)
		end
	end,
	contact=function(ply,ent,other)
		if ent:GetModel()==ent.formTable.model2 then
			return 25,DMG_SHOCK,20000
		end
	end
})
...
pk_pills.register("cityscanner",{
	...
	pose={
		...
		tail_control=function(ply,ent)
			return ent:GetPhysicsObject():GetVelocity().z/6;
		end
	},
	attack={
		mode= "trigger",
...
pk_pills.register("manhack",{
	...
	pose={
		Panel1=function(ply,ent,old)
			if ent.blades&&old<90 then
				return old+20
			end

			if !ent.blades&&old>0 then
				return old-20
			end
		end,
	...
	collide=function(ply,ent,collide)
		if ent.blades && collide.HitNormal.z < 0.5 && collide.HitNormal.z > -0.5 then
			...
			if (
				collide.HitEntity:IsPlayer() ||
				collide.HitEntity:IsNPC() ||
				collide.HitEntity:GetClass() == "pill_ent_phys"
			) then
...
pk_pills.register("cturret",{
	...
	sounds={
		...
		auto_ping_func=function(ply,ent)
			return ent.active&&!ent.loopingSounds["alarm"]:IsPlaying()&&(!ent.lastAttack||ent.lastAttack+.2<CurTime()),1
		end
	}
})

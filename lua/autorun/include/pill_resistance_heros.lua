...
pk_pills.register("hero_infiltrator",{
	...
	ammo={AR2=100,Buckshot=100,smg1=100},
	health=1000,
	...
	flashlight=function(ply,ent)
		...
		if ent.disguised then
			...
			timer.Simple(1,function()
				if IsValid(ent.disguise_faceplate) then ent.disguise_faceplate:Fire("setparentattachment","faceplate_attachment", 0) end
				if IsValid(ent) then pk_pills.setAiTeam(ply,"default") end
			end)

			timer.Simple(2,function()
				if IsValid(ent.disguise_faceplate) then ent.disguise_faceplate:Remove() end
				if IsValid(ent.disguise_helmet) then ent.disguise_helmet:Remove() end
			end)
			...
			timer.Simple(1,function()
				if IsValid(ent.disguise_faceplate) then ent.disguise_faceplate:Fire("setparentattachment","helmet_attachment", 0) end
				if IsValid(ent) then pk_pills.setAiTeam(ply,"hl_infiltrator") end
			end)
		end
	end
})
...
pk_pills.register("hero_hacker",{
	...
	flashlight=function(ply,ent)
		...
		if (tr.HitPos:Distance(tr.StartPos)<100 && table.HasValue(hackables,tr.Entity:GetClass())) && pk_pills.getAiTeam(tr.Entity)!="default" then

pk_pills.registerDrive("roll",{
	...
	key=function(ply,ent,options,key)
		if key==IN_JUMP&&options.jump then
			...
			if shouldJump then ent:GetPhysicsObject():ApplyForceCenter(Vector(0,0,options.jump)) ent:PillSound("jump") end
		elseif key==IN_DUCK&&options.burrow&&ent:GetMoveType()==MOVETYPE_VPHYSICS then
			local trace = util.QuickTrace(ent:GetPos(),Vector(0,0,-ent:BoundingRadius()+2),ent)
...
pk_pills.registerDrive("fly",{
	think=function(ply,ent,options)
		...
		if options.rocketMode then
			...
			if !options.tilt then
				if (ply:KeyDown(IN_FORWARD)) then move = rotatedAngle:Forward()*options.speed
				elseif (ply:KeyDown(IN_BACK)) then move = move + rotatedAngle:Forward()*-options.speed
				end
				
				if (ply:KeyDown(IN_MOVERIGHT)) then move = move + rotatedAngle:Right()*options.speed
				elseif (ply:KeyDown(IN_MOVELEFT)) then move = move + rotatedAngle:Right()*-options.speed
				end
				...
				if (ply:KeyDown(IN_FORWARD)) then move = baseDir:Forward()*options.speed aim.p=options.tilt||0
				elseif (ply:KeyDown(IN_BACK)) then move = move + baseDir:Forward()*-options.speed aim.p=-options.tilt||0
				end
				
				if (ply:KeyDown(IN_MOVERIGHT)) then move = move + baseDir:Right()*options.speed aim.r=options.tilt||0
				elseif (ply:KeyDown(IN_MOVELEFT)) then move = move + baseDir:Right()*-options.speed aim.r=-options.tilt||0
				end
			end

			//UpDown
			if (ply:KeyDown(IN_JUMP)) then move = move + Vector(0,0,options.speed*2/3)
			elseif (ply:KeyDown(IN_DUCK)) then move = move + Vector(0,0,-options.speed*2/3)
			end
		end
...
pk_pills.registerDrive("hover",{
	think=function(ply,ent,options)
		...
		if !ply:KeyDown(IN_DUCK) then
			...
			//Hovering
			local tr = util.QuickTrace(ent:GetPos(),Vector(0,0,-1)*(options.height||100),ent)
			if tr.Hit then
...
pk_pills.registerDrive("swim",{
	think=function(ply,ent,options)
		local phys=ent:GetPhysicsObject()
		if !ent.setupBuoyancy then phys:SetBuoyancyRatio(.135) ent.setupBuoyancy=true end
		...
		if (ply:KeyDown(IN_FORWARD)) then move = rotatedAngle:Forward()
		elseif (ply:KeyDown(IN_BACK)) then move = move - rotatedAngle:Forward()
		end
		
		if (ply:KeyDown(IN_MOVERIGHT)) then move = move + rotatedAngle:Right()
		elseif (ply:KeyDown(IN_MOVELEFT)) then move = move - rotatedAngle:Right()
		end
...
pk_pills.registerDrive("strider",{
	think=function(ply,ent,options)
		...
		if h<200 then h=200 ent:SetPoseParameter("body_height",h) end

		//UpDown
		if ply:KeyDown(IN_JUMP) && h<500 then h=h+5 ent:SetPoseParameter("body_height",h)
		elseif ply:KeyDown(IN_DUCK) && h>200 then h=h-5 ent:SetPoseParameter("body_height",h)
		end
		...
		local move=Vector(0,0,0)
		if (ply:KeyDown(IN_FORWARD)) then move = aim:Forward()
		elseif (ply:KeyDown(IN_BACK)) then move = move - aim:Forward()
		end
		
		if (ply:KeyDown(IN_MOVERIGHT)) then move = move + aim:Right()
		elseif (ply:KeyDown(IN_MOVELEFT)) then move = move - aim:Right()
		end
		...
		if (move:Length()>0) then
			...
			//Stepping
			//Step Sounds
			if !ent.lastStep then ent.lastStep=0 end
			
			if ent.lastStep==0 && ent:GetCycle()>1/3 then ent.lastStep=1 ent:PillSound("step")
			elseif ent.lastStep==1 && ent:GetCycle()>2/3 then ent.lastStep=2 ent:PillSound("step")
			elseif ent.lastStep==2 && ent:GetCycle()<2/3 then ent.lastStep=0 ent:PillSound("step")
			end
		else
			ent:SetPlaybackRate(1)
			ent:PillAnim("Idle01")
		...
		if tr.Hit then
			phys:AddVelocity(-ent:GetVelocity() + Vector(0,0,2000*(1-tr.Fraction)) + move*(run&&200||100))
		end
